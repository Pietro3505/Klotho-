...
class MateriasCalendarioTVC: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if notasParaLaFechaSeleccionada?[indexPath.row].completo == true {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
          if segue.identifier == "detallesPreguntaDesdeCalendario" {
              let detailsVC = segue.destination as! DetallesNotaVC
            let indexPath = tableView.indexPathForSelectedRow?.row
            detailsVC.actividadDetallada = notasParaLaFechaSeleccionada?[indexPath!]
          }
      }
    
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
     performSegue(withIdentifier: "detallesPreguntaDesdeCalendario", sender: self)
